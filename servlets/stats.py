...
class RankingsHandler(base_servlet.BaseRequestHandler):
    ...
    def get(self):
        ...
        style_event_rankings = rankings.compute_template_city_rankings(rankings.get_city_by_event_rankings(), rankings.STYLES, time_period)
        style_user_rankings = rankings.compute_template_city_rankings(rankings.get_city_by_user_rankings(), rankings.PEOPLES, time_period)
        ...
        if self.user:
            self.display['user_city'] = self.user.get_city()
