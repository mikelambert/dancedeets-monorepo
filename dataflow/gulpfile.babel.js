...
function remoteJob(filename) {
  return $.shell.task([`/usr/local/bin/python -m ${filename} --log=DEBUG --project dancedeets-hrd --job-name=${filename} --runner DataflowRunner --staging_location ${bucket}/staging --temp_location ${bucket}/temp --output ${bucket}/output --sdk_location ${sdk} --setup_file ./setup.py --num_workers=20`]);
}
