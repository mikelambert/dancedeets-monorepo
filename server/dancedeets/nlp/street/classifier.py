...
def is_vogue_event(classified_event):
    ...
    vogue_event_matches = set(classified_event.processed_text.get_tokens(keywords.VOGUE_EVENT))
    easy_vogue_matches = set(classified_event.processed_text.get_tokens(keywords.EASY_VOGUE, keywords.TOO_EASY_VOGUE))
    match_count = len(vogue_matches) + len(vogue_event_matches) + 0.34 * len(easy_vogue_matches)
