...
def count_event_for_city(dbevent):
    ...
    for time_period in get_time_periods(dbevent.creation_time or dbevent.start_time):
        yield op.counters.Increment(
            make_key_name("City", city=city, time_period=time_period)
        )
        yield op.counters.Increment(
            make_key_name("Country", country=dbevent.country, time_period=time_period)
        )
...
def count_user_for_city(user):
    ...
    for time_period in get_time_periods(user.creation_time):
        yield op.counters.Increment(
            make_key_name("City", city=user_city, time_period=time_period)
        )
...
def get_city_by_event_rankings(vertical):
    final_counter_map = _get_counter_map_for_ranking(
        _get_app_id(EVENT_FOR_CITY_RANKING, vertical)
    )
