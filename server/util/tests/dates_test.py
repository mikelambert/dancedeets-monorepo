...
class TestTimeHumanFormat(TestDates):
    def runTest(self):
        self.assertEqual(dates.time_human_format(self.nye), '12:00am')
        self.assertEqual(dates.time_human_format(self.nye, country='UK'), '0:00')
        self.assertEqual(dates.time_human_format(self.nye_plus_15), '3:00pm')
        self.assertEqual(dates.time_human_format(self.nye_plus_15, country='UK'), '15:00')


class TestDateHumanFormat(TestDates):
    def runTest(self):
        self.assertEqual(dates.date_human_format(self.nye), 'Saturday, January 1, 2000 - 12:00am')


class TestDurationHumanFormat(TestDates):
    def runTest(self):
        self.assertEqual(dates.duration_human_format(self.nye, self.nye_plus_8), 'Saturday, January 1, 2000 - 12:00am to 8:00am')
        self.assertEqual(
            dates.duration_human_format(self.nye, self.nye_plus_2_days_plus_8),
...
class TestEventDates(TestDates):
    def runTest(self):
        ...
        self.assertEqual(dates.parse_fb_start_time(e), datetime.datetime(2012, 4, 18, 5, 30))
        e = {'info': {'start_time': '2012-04-17T13:30:00'}}
        self.assertEqual(dates.parse_fb_start_time(e), datetime.datetime(2012, 4, 17, 13, 30))
