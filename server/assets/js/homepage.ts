...
interface JQueryWithBackstretch extends JQuery {
  backstretch(images: string[], options: { duration: number; fade: number }): void;
}
...
function setupHomepage(): void {
  ...
  if ((jQuery(document) as JQuery<Document>).width()! > 900) {
    images = images.map(x => x.replace('.jpg', '@2x.jpg'));
  }
  (jQuery('.fullscreen-static-image') as unknown as JQueryWithBackstretch).backstretch(images, {
    duration: 8000,
    fade: 1500,
  });
}
