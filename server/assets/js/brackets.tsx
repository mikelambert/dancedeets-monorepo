...
interface MatchReactProps {
  bracketRenderer: BracketRenderer;
  match: Match | null;
  position: Position;
}

interface MatchReactState {
  imageCounter: number;
}

class MatchReact extends React.Component<MatchReactProps, MatchReactState> {
  _intervalId: number = 0;

  constructor(props: MatchReactProps) {
    super(props);
  ...
  render(): React.ReactElement {
    ...
    if (this.props.match) {
      ...
      const contestants: Array<React.ReactNode> = [this.props.match.first, this.props.match.second];
      ...
      let textOverlay: React.ReactElement = (
        <div
          style={{
            position: 'absolute',
            width: '100%',
...
class BracketLines extends React.Component<BracketLinesProps> {
  render(): React.ReactElement {
    const box = {
...
interface BracketReactProps {
  bracket: Bracket;
}

class BracketReact extends React.Component<BracketReactProps> {
  render(): React.ReactElement {
    ...
    const bracketRenderer = new BracketRenderer(bracket as Bracket);
