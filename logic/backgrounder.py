...
def email_users(fb_uids, **kwargs):
    ...
    for i in range(0, len(fb_uids), task_size):
        taskqueue.add(method='GET', url='/tasks/email_user?' + urllib.urlencode(dict(user_id=','.join(str(x) for x in fb_uids[i:i+task_size]))), queue_name='slow-queue', **kwargs)
...
def load_users(fb_uids, allow_cache=True, **kwargs):
    ...
    for i in range(0, len(fb_uids), task_size):
        taskqueue.add(method='GET', url='/tasks/load_users?' + urllib.urlencode(dict(user_id=','.join(str(x) for x in fb_uids[i:i+task_size]), user_ids=','.join(str(x) for x in fb_uids[i:i+task_size]), allow_cache=allow_cache_arg)), queue_name='slow-queue', **kwargs)
