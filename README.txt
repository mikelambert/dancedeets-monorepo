...
This must be done after every change to the templates/ directory.

Finally, you can run ./nose.sh to run all of the included tests. Some of them actually talk to
google maps API server (and form a regression test to ensure we understand the returned data).
As such, if you run the tests too often you may encounter quota-exceeded errors.
Just wait a bit before re-running tests, or pass in a directory/filename to limit the scope
