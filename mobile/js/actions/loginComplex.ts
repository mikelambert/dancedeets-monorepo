...
export async function loginButtonPressed(dispatch: Dispatch): Promise<AccessToken | null> {
  try {
...
async function isRecentlyLoggedIn(): Promise<boolean> {
  const accessToken = await AccessToken.getCurrentAccessToken();
  if (accessToken != null) {
...
interface ExtendedAccessToken extends AccessToken {
  getPermissions(): string[];
}

async function refreshFullToken(): Promise<void> {
  ...
  try {
    ...
    const newAccessToken = await AccessToken.getCurrentAccessToken() as ExtendedAccessToken | null;
    ...
    console.log('Exception refreshing or logging in:', e, (e as Error).stack);
  }
}
