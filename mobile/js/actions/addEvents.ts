...
export function reloadAddEvents(): ThunkAction {
  return async (dispatch: Dispatch) => {
    ...
    try {
      ...
      console.log('error fetching events', e, (e as Error).stack);
...
function markAsPending(eventId: string): Action {
  return {
...
function markAsLoaded(eventId: string): Action {
  return {
    type: 'ADD_EVENTS_UPDATE_LOADED',
    status: 'LOADED',
    eventId,
...
function markAsUnLoaded(eventId: string): Action {
  return {
    type: 'ADD_EVENTS_UPDATE_LOADED',
    status: 'UNLOADED',
    eventId,
...
function alreadyProcessing(getState: GetState, eventId: string): boolean | undefined {
...
export function clickEvent(eventId: string): Action {
  return {
    type: 'ADD_EVENTS_UPDATE_LOADED',
    status: 'CLICKED',
    eventId,
