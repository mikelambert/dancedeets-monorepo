...
class FeaturedEvent extends React.PureComponent<FeaturedEventProps> {
  render() {
...
class FeaturedEvents extends React.PureComponent<FeaturedEventsProps> {
...
interface OneboxViewProps {
  onebox: Onebox;
}

class OneboxView extends React.PureComponent<OneboxViewProps> {
  constructor(props: OneboxViewProps) {
    super(props);
    this.oneboxClicked = this.oneboxClicked.bind(this);
  }
...
class _EventListContainer extends React.Component<EventListContainerProps> {
  constructor(props: EventListContainerProps) {
    super(props);
    this.setLocationAndSearch = this.setLocationAndSearch.bind(this);
    this.fetchLocationAndSearch = this.fetchLocationAndSearch.bind(this);
  }
  ...
  async authAndReloadProfile(address: string) {
    if (!isAuthenticated()) {
      return;
    }
    if (!address) {
...
const EventListContainer = connect(
  ...
  (dispatch: any) => ({
    setWaitingForLocationPermission: async (waiting: boolean) =>
      await dispatch(setWaitingForLocationPermission(waiting)),
    detectedLocation: async (location: string) => {
...
class _ListView extends React.Component<ListViewProps, ListViewState> {
  _loadingPeople: boolean = false;
  _listView: SectionList<any> | null = null;
  ...
  constructor(props: ListViewProps) {
    ...
    this.state = {
      failed: false,
      people: props.search.response ? props.search.response.people : null,
    };
  }
  ...
  componentWillReceiveProps(nextProps: ListViewProps) {
    ...
    if (
      ...
      if (
        listView &&
        (listView as any)._wrapperListRef.getListRef()._highestMeasuredFrameIndex
      ) {
  ...
  static getData(props: ListViewProps, state: ListViewState) {
  ...
  renderErrorView(error: string | null) {
    return (
