...
export class MediumBlog extends Blog {
  ...
  static async load(name: string): Promise<MediumBlog> {
...
export class FeedBlog extends Blog {
  ...
  static async load(url: string): Promise<FeedBlog> {
...
export class YoutubePlaylistBlog extends Blog {
  constructor(playlistJson: any, playlistItemsJson: any, videosJson: any) {
    ...
    const contentDetailsLookup: { [key: string]: any } = {};
    videosJson.items.forEach((x: any) => {
    ...
    const filteredPlaylistItems = playlistItemsJson.items.filter(
      (x: any) => contentDetailsLookup[x.snippet.resourceId.videoId]
    );

    this.posts = filteredPlaylistItems.map((x: any) =>
  ...
  static getUrl(path: string, args: { [key: string]: any }): string {
  ...
  static async getTimes(playlistItemsJson: any): Promise<any> {
  ...
  static async load(playlistId: string): Promise<YoutubePlaylistBlog> {
    const playlistItemsUrl = YoutubePlaylistBlog.getUrl(
