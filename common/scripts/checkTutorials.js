...
async function checkTutorial(tutorialJson) {
  ...
  const thumbnailVideoId = videoIds.find(
    videoId =>
      tutorialJson.thumbnail ===
      `https://i.ytimg.com/vi/${videoId}/mqdefault.jpg`
  );
  if (!thumbnailVideoId) {
    console.error(
      'Thumbnail for tutorial set incorrectly:',
      tutorialJson.style,
      tutorialJson.title
    );
  }
  ...
  for (const section of tutorialJson.sections) {
    for (const video of section.videos) {
      ...
      if (!foundVideo) {
        console.error(
          `Tutorial ${video.youtubeId} has broken video reference.`
        );
      ...
      if (!foundOEmbed) {
        console.error(
          `Tutorial ${video.youtubeId} has broken oembed video reference.`
        );
        continue;
        ...
        console.error(
          `Tutorial ${video.youtubeId} has incorrect dimensions: ${video.width}x${video.height}, expected: ${foundOEmbed.width}x${foundOEmbed.height}`
        );
        video.height = foundOEmbed.height;
        video.width = foundOEmbed.width;
      }
